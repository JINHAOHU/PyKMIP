...
class ProxyKmipClient(object):
    ...
    def encrypt(self, data, uid=None, cryptographic_parameters=None,
                iv_counter_nonce=None, auth_additional_data=None):
        """
        ...
        Returns:
            ...
            bytes: The auth tag that will be needed to authenticate the
                decrypted data.
        ...
        status = result.get('result_status')
        if status == enums.ResultStatus.SUCCESS:
            return result.get('data'), result.get('iv_counter_nonce'), result.get('auth_tag')
        else:
            raise exceptions.KmipOperationFailure(
                status,
                result.get('result_reason'),
